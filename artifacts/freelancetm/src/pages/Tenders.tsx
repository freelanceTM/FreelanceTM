...
import { Search, Clock, Users, ArrowRight, Briefcase } from "lucide-react";
import { useListTenders, useListCategories } from "@workspace/api-client-react";
    ...
    <Layout>
      ...
      <div className="container mx-auto max-w-5xl px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold">Open Projects</h2>
          <Button variant="outline" className="font-semibold border-border">
                  ...
                  <div className="flex flex-col md:flex-row gap-6">
                    <div className="flex-1 space-y-3">
                      <div className="flex items-center gap-2">
                        <Badge variant="secondary" className="bg-primary/10 text-primary hover:bg-primary/20 rounded-sm">
                          {tender.categoryName}
                        </Badge>
                        <span className="text-xs text-muted-foreground flex items-center">
                          <Clock className="h-3 w-3 mr-1" />
                          {tender.createdAt ? formatDistanceToNow(new Date(tender.createdAt), { addSuffix: true }) : "Recently"}
                        </span>
                      </div>
                      ...
                      {tender.skills && tender.skills.length > 0 && (
                        <div className="flex flex-wrap gap-2 pt-2">
                          {tender.skills.slice(0, 5).map(skill => (
                            <span key={skill} className="text-xs font-medium bg-muted px-2 py-1 rounded-md border text-muted-foreground">
                              {skill}
                            </span>
                          ))}
                    ...
                    <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4 md:w-48 shrink-0 md:border-l pl-0 md:pl-6 pt-4 md:pt-0 border-t md:border-t-0 mt-4 md:mt-0">
                      ...
                      <Button className="w-full mt-2 group bg-secondary text-secondary-foreground hover:bg-secondary/80">
                        View Details
